...
#### `configMap.storage.local.enabeld`
...
The `OIDC_PRIVATE_KEY` was created by running `docker run -u "$(id -u):$(id -g)" -v "$(pwd)":/keys authelia/authelia:latest authelia crypto pair rsa generate --bits 4096 --directory /keys`. The `OIDC_HMAC_SECRET`, `SESSION_ENCRYPTION_KEY`, `JWT_TOKEN` and `STORAGE_ENCRYPTION_KEY` are random alphanumeric strings with 64 or more characters created by `docker run authelia/authelia:latest authelia crypto rand --length 64 --charset alphanumeric`.
...
| Namespace  | `authelia`                                            |
| PVC        | `authelia-data`                                       |
| Secret     | `authelia-oidc`, `authelia-secrets`, `users-database` |
